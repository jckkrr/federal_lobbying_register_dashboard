...
def search_l2c(chosen_search_column, chosen_search_term, only_exgov, only_energy, inc_public_servants):
    
    df_search = get_l2c()
    
    if chosen_search_column != None and chosen_search_term != None:
        df_search = df_search.loc[df_search[chosen_search_column] == chosen_search_term] 
    
    if only_exgov == True:
        df_search = df_search.loc[df_search['Former Govt. Representative'] == 'Yes']
        
    if only_energy == True:
        df_search = df_search.loc[df_search['is_energy'] == 'Yes']
    ...
    if inc_public_servants == False:
        ...
        df_search['Former Govt. Representative'] = np.where(df_search['Previous Position'].isin(exclude_positions), 'No', df_search['Former Govt. Representative'])
            
    df_search = df_search.sort_values(by = ['is_energy', 'Former Govt. Representative', 'Cessation Year', 'Lobbyist Name'], ascending = [True, False, False, True])
    df_search = df_search[['Lobbyist Name', 'Job Title', 'Parent Organisation', 'Org ABN', 'Former Govt. Representative', 'Previous Position', 'Cessation Year', 'Client Name', 'Client ABN', 'is_energy']]
    
    df_search['Cessation Year'] = df_search['Cessation Year'].astype(float)
    
    st.dataframe(df_search.style.format(precision=0))
...
st.write('Data downloaded: 18 July 2025 | General public servants not counted as former government representatives')
...
search_l2c(chosen_search_column, chosen_search_term, only_exgov, only_energy, inc_public_servants)
